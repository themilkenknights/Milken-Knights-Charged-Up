...
public class MkSwerveTrain {
    ...
    public void updateSwerve() {
        ...
        vars.avgDistInches = (Math.abs(vars.posInchTL) + Math.abs(vars.posInchTR) + Math.abs(vars.posInchBL)
                + Math.abs(vars.posInchBR)) / 4.0;
    ...
    public void etherRCWFinder(double FWD, double STR, double RCW) {
        ...
        vars.avgDistTest = (vars.avgDistTest + ((Math.abs(vars.mod1Test) + Math.abs(vars.mod2Test)
                + Math.abs(vars.mod3Test) + Math.abs(vars.mod4Test)) / 4.0));
     ...
     *         flip value for the driving motor (as an array so it can return two
     *         things in one instead of two seperatly)
     * @author team 6624
     */
     ...
     * angled path of motion can be achieved
     * 
    ...
    public void etherAutoUpdate(double thetaTurn, double heading, double side) {
        ...
        double calcangle = side
                * ((heading) + (((-thetaTurn / 2) + (((vars.avgDistInches) / (vars.totalDistance)) * (thetaTurn)))));
        ...
        SmartDashboard.putNumber("avgdistsimilarity",
                vars.avgDistInches - (vars.avgDistTest * AUTO.measToPredictRatio));
        SmartDashboard.putNumber("calcangletestsimilarrity",
                ((heading) + (((-thetaTurn / 2) + (((vars.avgDistInches) / (vars.totalDistance)) * (thetaTurn)))))
                        % 360);
    ...
    public void updateMotionMagic() {
        setModuleDrive(ControlMode.MotionMagic, vars.magicDistance, vars.magicDistance, vars.magicDistance,
                vars.magicDistance);
